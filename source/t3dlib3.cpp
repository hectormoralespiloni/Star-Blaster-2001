...
int DMusic_Play_Loop(int id, int mi, int mf, int mt)
{
	int single_meassure;
	MUSIC_TIME MusicTime;
	MUSIC_TIME meassure_ini, meassure_end;
...
if (dm_midi[id].dm_segment && dm_midi[id].state!=MIDI_NULL)
   ...
   // play segment and force tracking of state variable
   dm_midi[id].dm_segment->GetLength(&MusicTime);

   single_meassure=MusicTime/mt;
   meassure_ini=single_meassure*mi;
   meassure_end=single_meassure*mf;

   dm_midi[id].dm_segment->SetLoopPoints(meassure_ini,meassure_end);
   dm_midi[id].dm_segment->SetRepeats(99);
   dm_perf->PlaySegment(dm_midi[id].dm_segment, 0, 0, &dm_midi[id].dm_segstate);
   dm_midi[id].state = MIDI_PLAYING;
